...
RSpec.describe "PublicWaterSystems", type: :request do
  describe "GET /public_water_systems" do
    ...
    context "with summary" do
      it "returns correct summary counts when a sort is active" do
        create(:public_water_system, open_health_viol: true, population_served_count: 1000)
        create(:public_water_system, open_health_viol: false, population_served_count: 500)
      ...
      it "returns correct summary counts for the filtered scope" do
        create(:public_water_system, open_health_viol: true, population_served_count: 1000, stusps: "VT")
        create(:public_water_system, open_health_viol: false, population_served_count: 500, stusps: "VT")
        create(:public_water_system, open_health_viol: false, population_served_count: 200, stusps: "OH")
  ...
  describe "GET /public_water_systems/:pwsid" do
    ...
    context "when requesting HTML" do
      it "returns JSON regardless of Accept header" do
        pws = create(:public_water_system)

        get "/public_water_systems/#{pws.pwsid}"

        expect(response).to have_http_status(:ok)
        expect(response.parsed_body["pwsid"]).to eq(pws.pwsid)
      end
    end
  end
end
