...
![Data table mock](../mocks/data_table_mock.png)
...
- [x] **Default sort**: Table loads sorted A→Z by utility name. All other column headers show ↕ icon.
- [x] **Sort ascending**: Click any sortable column header → ↑ icon appears; data re-sorts ascending.
- [x] **Sort descending**: Click the same header again → ↓ icon appears; data re-sorts descending.
- [x] **Sort reset (3-state)**: Click the same header a third time → ↕ icon returns; table reverts to `pws_name ASC` order.
- [x] **NULLS LAST**: Sort any column descending → rows with "—" appear at the BOTTOM, not the top. _(Covered by spec; visually confirm with a page jump to the last page on a sparse column like area_sq_miles)_
- [X] **Nil string display**: Find a system with no county or source-type data; confirm cell shows "—" not empty/NA/0.
- [x] **Nil boolean**: Find a system with nil wholesaler status; confirm cell shows "—" not "No". 
- [x] **False boolean**: A system with `is_wholesaler: false` shows "No".
- [x] **True boolean**: A system with `is_wholesaler: true` shows "Yes".

### P2 — Layout and UX

- [x] **Sticky headers**: Scroll down → column headers stay fixed at the top of the viewport.
- [x] **Sticky first column**: Scroll right → "Utility Name" column stays fixed at the left edge.
- [x] **Corner cell**: Scroll both right and down → the "Utility Name" header stays pinned at the top-left corner (both sticky axes active simultaneously).
- [X] **No wrapping headers**: Every column header displays on a single line. In particular: "EPA Facility Report", "Facility type (School or daycare)", and all long violation headers (e.g., "Synthetic organic chemicals violations (5yr)") are one line.
- [x] **Column highlight**: Click a sort column → that column's `td` cells show a subtle blue tint. Click to reset sort → tint disappears.
- [x] **Scrollbars styled**: Horizontal and vertical scrollbars are visibly styled (not the browser-default near-invisible thin bars).
- [x] **Row striping**: Even rows have a light gray background; odd rows are white.
- [x] **Row hover**: Hovering over any row shows a blue-tinted row highlight.
- [x] **Empty state**: Apply filters that return 0 results → shows "No results match the current filters." _(HTML is in place — verify visually with an impossible filter combo)_
- [x] **Pagination**: 3-column pinned footer — "Showing X to Y of Z entries" left, «/‹/[page input]/›/» centered, Map/Table toggle right. Jump-to-page input navigates directly. All elements on the same horizontal level, does not overlap table rows.
- [x] **Single page**: With ≤50 rows, shows "Showing N entries" with no nav buttons.
