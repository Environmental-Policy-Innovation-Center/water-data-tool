...
|---|---|---|---|
| 1 | Gem security audit | `bin/bundler-audit` | `scan_ruby` |
| 2 | Ruby static analysis | `bin/brakeman` | `scan_ruby` |
| 3 | JS dependency audit | `bin/importmap audit` | `scan_js` |
| 4 | Ruby style | `bin/standardrb` | `lint` |
| 5 | ERB style | `bin/erb_lint --lint-all` | *(bin/ci + commit hook only)* |
| 6 | Tests | `bundle exec rspec` | `test` |

> Run any single check on its own with the command in the table — you don't have to re-run all of `bin/ci` to check one fix.

---

## 1. Gem security audit — `bin/bundler-audit`
...
## 2. Ruby static analysis — `bin/brakeman`
...
## 3. JS dependency audit — `bin/importmap audit`
...
## 5. ERB style — `bin/erb_lint --lint-all`
